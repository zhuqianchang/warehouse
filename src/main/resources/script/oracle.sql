...
comment on column W_OPERATIONSTOCK.QUANTITY
  is '数量';
comment on column W_OPERATIONSTOCK.PRICE
  is '单价';
comment on column W_OPERATIONSTOCK.OPERATIONTYPE
  is '操作类型';
comment on column W_OPERATIONSTOCK.STOVECODE
  is '炉号';
...
(
  PRODUCECODE    NVARCHAR2(40) not null,
  PRODUCTIONCODE NVARCHAR2(40) not null,
...
;
comment on table W_PRODUCE
  is '采购清单表';
comment on column W_PRODUCE.PRODUCECODE
  is '生产编号';
comment on column W_PRODUCE.PRODUCTIONCODE
  is '成品编号';
comment on column W_PRODUCE.PRODUCEDATE
  is '生产日期';
comment on column W_PRODUCE.QUANTITY
  is '数量';
comment on column W_PRODUCE.CREATEUSER
  is '创建人';
comment on column W_PRODUCE.CREATEDATETIME
  is '创建时间';
comment on column W_PRODUCE.MODIFYUSER
  is '修改人';
comment on column W_PRODUCE.MODIFYDATETIME
  is '修改时间';
alter table W_PRODUCE
  add constraint PRODUCE_FK1 foreign key (PRODUCTIONCODE)
  references W_PRODUCTION (PRODUCTIONCODE);