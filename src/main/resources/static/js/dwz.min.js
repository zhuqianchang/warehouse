var DWZ = {
    ...
    ajaxDone: function (json) {
        if (json[DWZ.keys.statusCode] == DWZ.statusCode.error) {
            ...
            if (json[DWZ.keys.statusCode] == DWZ.statusCode.timeout) {
                if (alertMsg) {
                    alertMsg.error(json[DWZ.keys.message] || DWZ.msg("sessionTimout"), {okCall: DWZ.loadLogin})
                } else {
                    DWZ.loadLogin()
                }
            } else {
    ...
    init: function (pageFrag, options) {
        ...
        $.extend(DWZ.ui, op.ui);
        jQuery.ajax({
            type: "GET", url: pageFrag, dataType: "xml", timeout: 50000, cache: false, error: function (xhr) {
    ...
    $.extend($.dwz.accordion, {
        defaults: {
            ...
            slide: function (options, additions) {
                ...
                var hideHeight = options.toHide.height(), showHeight = options.toShow.height(),
    ...
    if ($.fn.validate) {
        $("form.required-validate", $p).each(function () {
            ...
            $form.find("input[customvalid]").each(function () {
                var $input = $(this);
                $input.rules("add", {customvalid: $input.attr("customvalid")})
            })
        })
    }
    if ($.fn.datepicker) {
...
(function ($) {
    $.fn.extend({
        theme: function (options) {
            var op = $.extend({themeBase: "theme"}, options);
    ...
    $.fn.extend({
        tabs: function (options) {
            ...
            function initTab(jT) {
                ...
                var collapse = jT.hasClass("collapse"), $tabsHeaderContent = jTabHeader.find(">.tabsHeaderContent");
...
(function ($) {
    $.pdialog = {
        _op: {
            ...
            minable: true,
            fresh: true
        }, _current: null, _zIndex: 42, getCurrent: function () {
...
(function ($) {
    ...
    DWZ.sortDrag = {
        _onDrag: false, start: function ($sortBox, $item, event, op) {
            var me = this;
            if (me._onDrag) {
                setTimeout(function () {
                    me._onDrag = false
            ...
            }).jDrag({selector: op.selector, drag: this.drag, stop: this.stop, event: event});
            ...
            var itemPos = $item.position(), y = itemPos.top + ($item.height() / 2),
                x = itemPos.left + ($item.width() / 2);
            var op = $.extend({}, _op, $item.data("op"));
            return $(op.sortBoxs).filter(":visible").filter(function () {
                var $sortBox = $(this), sortBoxPos = $sortBox.position(), sortBoxH = $sortBox.height(),
...
(function ($) {
    ...
    $.jTableTool = {
        ...
        // 解决tbody追加行出现的列宽不一致的问题，解决列宽拖拽的前提条件
        initGridColumnWidth:function($grid){
            var $ths = $(".gridThead thead>tr:first>th", $grid), $trs = $(".gridTbody tbody>tr:first>td", $grid);
...
DWZ.pargerFormExport = function (url, $form) {
    ...
    var pagerFormUrl = $form[0].action, pagerFormOnSubmit = $form.attr("onsubmit");
    $form[0].action = url;
...
(function ($) {
    ...
    $.extend({
        bringBackSuggest: function (args) {
            ...
            if (_lookup.callback){
                if($.isFunction(_lookup.callback)) {
                    _lookup.callback(args);
                } else {
                    try {
                        eval('(' + _lookup.callback + ')')(args);
                    } catch (e) {
            ...
            return this.each(function () {
                ...
                $this.click(function (event) {
                    _lookup = $.extend(_lookup, {
                        ...
                        callback: $this.attr("callback") || null,
...
(function ($) {
    ...
    $.checkbox = {
        selectAll: function (_name, _parent) {
            ...
            var $parent = $(_parent || document), $checkboxLi = $parent.find(":checkbox[name='" + _name + "']");
            switch (_type) {
                case"invert":
...
(function ($) {
    ...
    var _comboxReset = function ($select) {
        var $box = $select.parents("div.select:first"), defaultValue = $box.find(">a").attr("default-value");
